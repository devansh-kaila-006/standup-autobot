...
export interface DashboardState {
    ...
    heatmapData: number[]; // Array of 7 numbers (0.0 to 1.0)
}
...
export class DashboardPanel {
    ...
    public static createOrShow(extensionUri: vscode.Uri, state: DashboardState) {
    ...
    public static revive(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, state: DashboardState) {
        DashboardPanel.currentPanel = new DashboardPanel(panel, extensionUri, state);
    }

    public static update(state: DashboardState) {
        if (DashboardPanel.currentPanel) {
            DashboardPanel.currentPanel._update(state);
        }
    }

    private constructor(panel: vscode.WebviewPanel, private readonly _extensionUri: vscode.Uri, state: DashboardState) {
        this._panel = panel;
        this._update(state);
    ...
    private _update(state: DashboardState) {
        this._panel.webview.html = this._getHtmlForWebview(state);
    }

    private _getHtmlForWebview(state: DashboardState) {
