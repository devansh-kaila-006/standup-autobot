...
const logger = new Logger_1.Logger('HarvestService');
class HarvestService {
    ...
    loadConfig() {
        ...
        this.config = {
            accountId: config.get('harvestAccountId', ''),
     ...
     * Create time entry
     */
    async createTimeEntry(projectId, taskId, spentDate, hours, notes) {
        if (!this.config || !this.config.accessToken || !this.config.accountId) {
            logger.warn('Harvest not configured');
            return null;
        }
     ...
     */
    async makeHarvestRequest(url, method, data) {
        if (!this.config || !this.config.accessToken || !this.config.accountId) {
