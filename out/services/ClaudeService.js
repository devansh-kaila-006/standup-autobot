...
const logger = new Logger_1.Logger('ClaudeService');
class ClaudeService {
    constructor(context) {
    ...
    loadConfig() {
        ...
        this.config = {
            ...
            baseURL: config.get('claudeBaseUrl', 'https://api.anthropic.com'),
            model: config.get('claudeModel', 'claude-3-5-sonnet-20241022'),
            temperature: config.get('claudeTemperature', 0.7),
            maxTokens: config.get('claudeMaxTokens', 1000),
        };
        this.loadApiKey();
    }
    /**
...
`;
     ...
     */
    async makeMessageRequest(systemPrompt, userPrompt) {
        if (!this.config || !this.config.apiKey) {
     ...
     */
    async makeClaudeRequest(url, method, data) {
        if (!this.config || !this.config.apiKey) {
            throw new Error('Claude not configured');
        }
    ...
    async testConnection() {
        ...
        try {
            const response = await this.makeMessageRequest('Hello', 'Say "Connection successful" if you receive this.');
