...
class SmartFeaturesService {
     ...
     */
    categorizeFileActivity(filePath) {
     ...
     */
    categorizeTerminalActivity(command) {
        ...
        recentLogs.forEach((log) => {
            if (log.activities) {
                log.activities.forEach((activity) => {
                    ...
                    patternFrequency.set(category.category, (patternFrequency.get(category.category) || 0) + 1);
                });
            }
        });
     ...
     */
    getActivityDescription(category) {
        const descriptions = {
     ...
     */
    recordCorrection(originalCategory, correctedCategory) {
     ...
     */
    async getEnhancedStandupSuggestions(data) {
     ...
     */
    loadPatterns() {
        const stored = this.context.globalState.get('standup.activityPatterns');
        if (stored) {
...
//# sourceMappingURL=SmartFeaturesService.js.map