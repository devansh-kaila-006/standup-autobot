...
describe('Error Classes', () => {
    describe('StandupError', () => {
        it('should create base error with message and code', () => {
            const error = new errors_1.StandupError('Test error', 'TEST_CODE');
        ...
        it('should include details', () => {
            ...
            const error = new errors_1.StandupError('Test error', 'TEST_CODE', details);
            expect(error.details).toEqual(details);
        });
        it('should convert to JSON', () => {
            const error = new errors_1.StandupError('Test error', 'TEST_CODE', { key: 'value' });
    ...
    describe('APIError', () => {
        it('should create API error with status code', () => {
            const error = new errors_1.APIError('API failed', 404, '/api/test');
    ...
    describe('ExportError', () => {
        ...
        it('should include export type in user message', () => {
            const error = new errors_1.ExportError('API error', 'jira');
        ...
        it('should handle missing export type', () => {
            const error = new errors_1.ExportError('General export error');
...
describe('Error Helper Functions', () => {
    describe('isStandupError', () => {
        it('should return true for StandupError instances', () => {
            const error = new errors_1.APIError('Test', 500);
            expect((0, errors_1.isStandupError)(error)).toBe(true);
        });
        it('should return false for regular errors', () => {
            const error = new Error('Regular error');
            expect((0, errors_1.isStandupError)(error)).toBe(false);
        });
        it('should return false for non-error values', () => {
            expect((0, errors_1.isStandupError)('string')).toBe(false);
            expect((0, errors_1.isStandupError)(null)).toBe(false);
            expect((0, errors_1.isStandupError)(undefined)).toBe(false);
        });
    });
    describe('getErrorMessage', () => {
        it('should get user message from StandupError', () => {
            const error = new errors_1.APIError('API failed', 401);
            expect((0, errors_1.getErrorMessage)(error)).toContain('Authentication failed');
        });
        it('should get message from regular Error', () => {
            const error = new Error('Regular error');
            expect((0, errors_1.getErrorMessage)(error)).toBe('Regular error');
        });
        it('should convert non-error to string', () => {
            expect((0, errors_1.getErrorMessage)('string error')).toBe('string error');
            expect((0, errors_1.getErrorMessage)(123)).toBe('123');
        });
    });
    describe('toStandupError', () => {
        it('should return StandupError as-is', () => {
            const original = new errors_1.APIError('Test', 500);
            const converted = (0, errors_1.toStandupError)(original);
            expect(converted).toBe(original);
        });
        it('should convert regular Error to StandupError', () => {
            const original = new Error('Regular error');
            const converted = (0, errors_1.toStandupError)(original);
            expect((0, errors_1.isStandupError)(converted)).toBe(true);
        ...
        it('should convert string to StandupError', () => {
            const converted = (0, errors_1.toStandupError)('String error');
            expect((0, errors_1.isStandupError)(converted)).toBe(true);
            expect(converted.message).toBe('String error');
            expect(converted.code).toBe('UNKNOWN_ERROR');
        });
        it('should convert null to StandupError', () => {
            const converted = (0, errors_1.toStandupError)(null);
            expect((0, errors_1.isStandupError)(converted)).toBe(true);
        });
    });
});
