...
describe('Logger', () => {
    let logger;
    beforeEach(() => {
        logger = new Logger_1.Logger('TestLogger', Logger_1.LogLevel.DEBUG);
    });
    afterEach(() => {
        logger.dispose();
    });
    describe('constructor and initialization', () => {
        it('should create logger with name and default level', () => {
            ...
            expect(testLogger.getLevel()).toBe(Logger_1.LogLevel.INFO);
            testLogger.dispose();
        });
        it('should create logger with custom level', () => {
            const debugLogger = new Logger_1.Logger('DebugTest', Logger_1.LogLevel.DEBUG);
            expect(debugLogger.getLevel()).toBe(Logger_1.LogLevel.DEBUG);
            debugLogger.dispose();
        });
    });
    describe('log levels', () => {
        it('should set and get log level', () => {
            logger.setLevel(Logger_1.LogLevel.WARN);
            expect(logger.getLevel()).toBe(Logger_1.LogLevel.WARN);
        });
        it('should not log DEBUG messages when level is INFO', () => {
            logger.setLevel(Logger_1.LogLevel.INFO);
            logger.debug('This should not be logged');
            const logs = logger.getLogs();
            const debugLogs = logs.filter(log => log.level === Logger_1.LogLevel.DEBUG);
        ...
        it('should log INFO messages when level is INFO', () => {
            ...
            const logs = logger.getLogs();
            const infoLogs = logs.filter(log => log.level === Logger_1.LogLevel.INFO);
            expect(infoLogs.length).toBe(1);
            expect(infoLogs[0].message).toBe('This should be logged');
        });
        it('should log ERROR messages regardless of level', () => {
            logger.setLevel(Logger_1.LogLevel.ERROR);
            logger.error('This should be logged');
            const logs = logger.getLogs();
            const errorLogs = logs.filter(log => log.level === Logger_1.LogLevel.ERROR);
    ...
    describe('log retrieval', () => {
        ...
        it('should filter logs by level', () => {
            const errorAndWarnLogs = logger.getLogsByLevel(Logger_1.LogLevel.WARN);
    ...
    describe('global logger functions', () => {
        ...
        it('should initialize global logger', () => {
            const initialized = (0, Logger_1.initLogger)(Logger_1.LogLevel.WARN);
            expect(initialized).toBeDefined();
            expect(initialized.getLevel()).toBe(Logger_1.LogLevel.WARN);
        });
        it('should get existing global logger', () => {
            const first = (0, Logger_1.initLogger)(Logger_1.LogLevel.DEBUG);
            const second = (0, Logger_1.getLogger)();
            expect(first).toBe(second);
        });
        it('should create named logger', () => {
            const namedLogger = (0, Logger_1.createLogger)('NamedLogger', Logger_1.LogLevel.ERROR);
            expect(namedLogger).toBeDefined();
            expect(namedLogger.getLevel()).toBe(Logger_1.LogLevel.ERROR);
            namedLogger.dispose();
        });
