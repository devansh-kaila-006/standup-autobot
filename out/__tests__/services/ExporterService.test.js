...
describe('ExporterService', () => {
    let exporterService;
    beforeEach(() => {
        exporterService = new ExporterService_1.ExporterService();
        jest.clearAllMocks();
    });
    ...
    describe('exportToEmail', () => {
        it('should create mailto link and open it', async () => {
            ...
            vscode.Uri.parse.mockReturnValue(mockUri);
            vscode.env.openExternal.mockResolvedValue(true);
            await exporterService.exportToEmail(markdown);
            expect(vscode.Uri.parse).toHaveBeenCalledWith(expect.stringContaining('mailto:'));
            expect(vscode.Uri.parse).toHaveBeenCalledWith(expect.stringContaining('subject='));
            expect(vscode.Uri.parse).toHaveBeenCalledWith(expect.stringContaining('body='));
            expect(vscode.env.openExternal).toHaveBeenCalledWith(mockUri);
        });
        it('should include date in subject', async () => {
            const markdown = 'Test';
            const dateStr = new Date().toLocaleDateString();
            vscode.Uri.parse.mockReturnValue({ toString: () => '' });
            vscode.env.openExternal.mockResolvedValue(true);
            await exporterService.exportToEmail(markdown);
            const callArgs = vscode.Uri.parse.mock.calls[0][0];
    ...
    describe('exportToNotion', () => {
        ...
        it('should export markdown to Notion successfully', async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith('https://api.notion.com/v1/blocks/test-page-id/children', expect.objectContaining({
                method: 'PATCH',
        ...
        it('should throw error when token is missing', async () => {
            ...
            await expect(exporterService.exportToNotion(markdown, config))
    ...
    describe('exportToJira', () => {
        ...
        it('should export markdown to Jira successfully', async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith('https://test-company.atlassian.net/rest/api/2/issue/PROJ-123/comment', expect.objectContaining({
                method: 'POST',
                headers: expect.objectContaining({
        ...
        it('should throw error when domain is missing', async () => {
            ...
            await expect(exporterService.exportToJira(markdown, config))
...
//# sourceMappingURL=ExporterService.test.js.map