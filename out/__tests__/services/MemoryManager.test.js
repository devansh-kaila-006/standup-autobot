...
describe('MemoryManager', () => {
    ...
    describe('cleanupOldData', () => {
        it('should remove entries older than retention period', async () => {
            ...
            mockContext.globalState.get.mockReturnValue(mockActivityLog);
            const result = await memoryManager.cleanupOldData(mockContext);
            expect(result.removedEntries).toBe(2);
            expect(result.retentionDays).toBe(30);
            expect(mockContext.globalState.update).toHaveBeenCalledWith('standup.activityLog', expect.objectContaining({
        ...
        it('should use default retention period when not configured', async () => {
            const mockConfig = {
                get: jest.fn((key) => key === 'dataRetentionDays' ? undefined : 30),
            };
            mockGetConfiguration.mockReturnValue(mockConfig);
    ...
    describe('Data Aggregation', () => {
        ...
        it('should handle empty activity log', () => {
            mockContext.globalState.get.mockReturnValue(null);
            const result = memoryManager.aggregateDataByDateRange(mockContext, new Date('2024-01-01'), new Date('2024-01-31'));
            expect(result).toEqual([]);
        });
    });
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle cache operations with null keys', () => {
            expect(() => {
                memoryManager.setCache(null, { data: 'value' });
            }).not.toThrow();
            expect(() => {
                memoryManager.getCache(null);
            }).not.toThrow();
        });
