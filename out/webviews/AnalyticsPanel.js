...
class AnalyticsPanel {
    static createOrShow(extensionUri, context) {
        ...
        const panel = vscode.window.createWebviewPanel('standupAutobot.analytics', 'Analytics Dashboard', column || vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async _update(extensionUri) {
        ...
        this._panel.webview.html = this._getHtmlForWebview(this._panel.webview, productivityMetrics, trendData, weekOverWeek, rollingAverage);
    }
    async exportCSV() {
        try {
            const csv = await this.analyticsService.exportAnalyticsReport(30);
            const uri = await vscode.env.openExternal(vscode.Uri.parse(`data:text/csv;charset=utf-8,${encodeURIComponent(csv)}`));
...
${summary.highlights.map(h => `  • ${h}`).join('\n')}
            ...
            vscode.window.showErrorMessage('Failed to generate sprint summary: ' + error.message);
        }
    }
...
${report.recommendations.map(r => `  • ${r}`).join('\n')}
    ...
    _getHtmlForWebview(webview, metrics, trendData, weekOverWeek, rollingAverage) {
                ...
                <div class="container">
                    ...
                    <div class="metrics-grid">
                        ...
                        <div class="metric-card">
                            ...
                            <div class="value">${metrics.mostProductiveHours.map((h) => `${h}:00`).join(', ')}</div>
